var icBgAnimation = function() {
	...
	function initTop() {
		...
		for (var i in points) {
			var c = new Circle(points[i], 2 + Math.random() * 6, "rgba(255,255,255,1)");
			points[i].circle = c;
		}
	}
	...
	function drawLines(p) {
		...
		for (var i in p.closest) {
			...
			ctx.strokeStyle = "rgba(215,215,255,1" + p.active + ")";
			ctx.stroke();
		}
	}
	...
	function Circle(pos, rad, color) {
		...
		this.draw = function() {
			...
			ctx.fillStyle = "rgba(246,241,200,1" + _this.active + ")";
			ctx.fill();
		};
	}
